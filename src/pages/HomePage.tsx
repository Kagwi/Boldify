    ...
    <div className="min-h-screen bg-[#F8F6F2] relative overflow-x-hidden">
      ...
      <div ref={heroRef} className="relative h-screen overflow-hidden cursor-grab active:cursor-grabbing" onMouseMove={handleMouseMove}>
        {heroSlides.map((slide, index) => {
          ...
          return (
            <div key={index} className={`absolute inset-0 transition-all duration-1000 ease-out ${index === currentSlide ? 'opacity-100 scale-100' : 'opacity-0 scale-110'}`} style={{ transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)' }}>
              <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-black/40 z-10" />
              <img src={slide.image} alt={slide.title} className="w-full h-full object-cover object-center will-change-transform" style={{ transform: `translate(${offsetX}px, ${offsetY}px)`, transition: 'transform 0.1s ease-out' }} />
        ...
        <button onClick={prevSlide} className="absolute left-6 top-1/2 -translate-y-1/2 z-30 bg-black/40 backdrop-blur-sm hover:bg-white/90 text-white hover:text-black p-3 transition-all duration-300 rounded-full focus:outline-none" aria-label="Previous slide"><ChevronLeft className="h-6 w-6" /></button>
        <button onClick={nextSlide} className="absolute right-6 top-1/2 -translate-y-1/2 z-30 bg-black/40 backdrop-blur-sm hover:bg-white/90 text-white hover:text-black p-3 transition-all duration-300 rounded-full focus:outline-none" aria-label="Next slide"><ChevronRight className="h-6 w-6" /></button>
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex space-x-2">
          {heroSlides.map((_, index) => (
            <button key={index} onClick={() => { setCurrentSlide(index); setIsAnimating(true); setTimeout(() => setIsAnimating(false), 1000); }} className={`transition-all duration-500 rounded-full ${index === currentSlide ? 'w-8 h-1 bg-white' : 'w-2 h-1 bg-white/50 hover:bg-white/80'}`} aria-label={`Go to slide ${index + 1}`} />
          ))}
        </div>
      </div>
      ...
      <section ref={(el) => (sectionsRef.current[0] = el)} className="py-12 px-6 md:px-12 lg:px-24" style={{ backgroundColor: '#000000' }}>
        <div className="max-w-7xl mx-auto">
          <div ref={(el) => (headingRefs.current[0] = el)} className="mb-12 text-center">
            <h2 className="text-4xl md:text-5xl font-semibold text-white mb-4 tracking-tight" style={{ fontFamily: 'Jolt, serif' }}>Shop by Category</h2>
            <p className="text-gray-300 text-lg font-light max-w-2xl mx-auto" style={{ fontFamily: 'Playfair Display, serif' }}>Find your perfect piece by exploring our curated collections</p>
          </div>
          {loadingCategories ? (
            <div className="text-center text-white py-12"><p className="text-gray-300">Loading categories...</p></div>
          ) : categories.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
              {categories.map((category) => (
                <div key={category.id} onClick={() => navigateToCategory(category.slug)} className="group cursor-pointer overflow-hidden rounded-lg shadow-lg transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
                  <div className="relative h-64 overflow-hidden">
                    <img src={categoryImages[category.id] || 'https://images.pexels.com/photos/6174221/pexels-photo-6174221.jpeg?auto=compress&cs=tinysrgb&w=800'} alt={category.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 text-center">
                      ...
                      <p className="text-white/80 text-sm" style={{ fontFamily: 'Marcellus, serif' }}>Shop Now →</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center text-white py-12"><p className="text-gray-300">No categories found.</p></div>
          )}
        </div>
      </section>
      ...
      <section ref={(el) => (sectionsRef.current[1] = el)} className="py-12 px-6 md:px-12 lg:px-24">
        <div className="max-w-7xl mx-auto">
          <div ref={(el) => (headingRefs.current[1] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-[#1A1A1A] mb-4 tracking-tight" style={{ fontFamily: 'Jolt, serif' }}>Featured Collections</h2>
            <p className="text-[#4A4A4A] text-lg font-light max-w-2xl mx-auto" style={{ fontFamily: 'Playfair Display, serif' }}>Handpicked pieces that define boldness</p>
          </div>
      ...
      <section ref={(el) => (sectionsRef.current[2] = el)} className="py-12 px-6 md:px-12 lg:px-24 bg-[#F1EFEA]">
        <div className="max-w-7xl mx-auto">
          <div ref={(el) => (headingRefs.current[2] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-[#1A1A1A] mb-4" style={{ fontFamily: 'Jolt, serif' }}>New Arrivals</h2>
      ...
      <section ref={(el) => (sectionsRef.current[3] = el)} className="py-12 px-6 md:px-12 lg:px-24 bg-[#0A192F]">
        <div className="max-w-7xl mx-auto">
          <div ref={(el) => (headingRefs.current[3] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-white mb-4" style={{ fontFamily: 'Jolt, serif' }}>The Boldify Experience</h2>
            <p className="text-gray-300 text-lg font-light max-w-2xl mx-auto" style={{ fontFamily: 'Playfair Display, serif' }}>More than jewelry — a celebration of you</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              { icon: Gem, title: 'Ethically Sourced', description: 'We partner with suppliers who prioritize fair labor and sustainable practices, ensuring every gem and metal tells a story of integrity.', delay: 0 },
              { icon: Award, title: 'Artisan Crafted', description: 'Each piece is meticulously handcrafted by skilled artisans who pour passion and precision into every detail.', delay: 150 },
              { icon: Heart, title: 'Timeless Design', description: 'Modern silhouettes meet classic elegance – jewelry that transcends trends and becomes a cherished heirloom.', delay: 300 },
            ].map((item, idx) => (
              <div key={idx} ref={(el) => (experienceCardsRef.current[idx] = el)} className="text-center opacity-0 translate-y-6 transition-all duration-700 group" style={{ transitionDelay: `${item.delay}ms` }}>
                ...
                <h3 className="text-xl font-medium text-white mb-3" style={{ fontFamily: 'Playfair Display, serif' }}>{item.title}</h3>
                <p className="text-gray-300 text-sm leading-relaxed max-w-sm mx-auto" style={{ fontFamily: 'Marcellus, serif' }}>{item.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Customer Reviews */}
      <section ref={(el) => (sectionsRef.current[4] = el)} className="py-12 px-6 md:px-12 lg:px-24">
        <div className="max-w-7xl mx-auto">
          <div ref={(el) => (headingRefs.current[4] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-[#1A1A1A] mb-4" style={{ fontFamily: 'Jolt, serif' }}>Customer Reviews</h2>
            <p className="text-[#4A4A4A] text-lg font-light max-w-2xl mx-auto" style={{ fontFamily: 'Playfair Display, serif' }}>What our bold women say</p>
          </div>
          {loadingReviews ? (
            <div className="text-center py-12"><p className="text-gray-500">Loading reviews...</p></div>
          ) : (
      ...
      {/* About Section */}
      <section id="about" ref={(el) => { sectionsRef.current[5] = el; aboutRef.current = el; }} className="relative py-12 px-6 md:px-12 lg:px-24 bg-cover bg-center" style={{ backgroundImage: "url('https://images.pexels.com/photos/6387626/pexels-photo-6387626.jpeg?auto=compress&cs=tinysrgb&w=1920')" }}>
        <div className="absolute inset-0 bg-black/70"></div>
        <div className="relative z-10 max-w-6xl mx-auto">
          <div ref={(el) => (headingRefs.current[5] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-white mb-4" style={{ fontFamily: 'Jolt, serif' }}>About Boldify Jewelry</h2>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="text-gray-200 space-y-6 text-lg leading-relaxed" style={{ fontFamily: 'Marcellus, serif' }}>
              <p>At Boldify Jewelry.Ke, we believe that every woman deserves to wear pieces that reflect her strength, confidence, and unique style. Our carefully curated collection features bold, statement jewelry designed for the modern Kenyan woman who isn't afraid to stand out.</p>
              <p>From elegant necklaces to stunning earrings and exquisite bangles, each piece in our collection is selected with meticulous attention to detail. We combine luxury with affordability, ensuring that you can express your bold personality without compromise.</p>
              <p>Our mission is simple: to empower women through jewelry that makes them feel unstoppable. Whether you're dressing up for a special occasion or adding a touch of elegance to your everyday look, Boldify has the perfect piece for you.</p>
            </div>
            <div className="relative h-[400px] rounded-lg overflow-hidden shadow-xl">
              <img src="https://images.pexels.com/photos/6174221/pexels-photo-6174221.jpeg?auto=compress&cs=tinysrgb&w=1200" alt="Jewelry" className="w-full h-full object-cover" />
      ...
      {/* FAQ Section */}
      <section ref={(el) => (sectionsRef.current[6] = el)} className="relative py-12 px-6 md:px-12 lg:px-24 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center opacity-10" style={{ backgroundImage: "url('https://images.pexels.com/photos/6387626/pexels-photo-6387626.jpeg?auto=compress&cs=tinysrgb&w=1920')" }}></div>
        <div className="absolute inset-0 bg-gradient-to-b from-[#C4A747]/40 via-white/30 to-black/60"></div>
        <div className="absolute inset-0 opacity-30 pointer-events-none animate-smoke"><div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20viewBox=%220%200%20200%20200%22%3E%3Cfilter%20id=%22smoke%22%3E%3CfeTurbulence%20baseFrequency=%220.01%22%20numOctaves=%223%22%20seed=%221%22/%3E%3CfeColorMatrix%20values=%220%200%200%200%200%200%200%200%200%200%200%200%200%200%200%200%200%200%200.3%200%22/%3E%3C/filter%3E%3Crect%20width=%22100%25%22%20height=%22100%25%22%20filter=%22url(%23smoke)%22/%3E%3C/svg%3E')] bg-repeat bg-[length:400px] mix-blend-overlay animate-smokeMove"></div></div>
        <div className="absolute inset-0 bg-radial-gradient from-[#C4A747]/20 via-transparent to-transparent opacity-40"></div>
        <div className="relative z-10 max-w-4xl mx-auto">
          <div ref={(el) => (headingRefs.current[6] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-white drop-shadow-lg" style={{ fontFamily: 'Jolt, serif' }}>Frequently Asked Questions</h2>
            <p className="text-gray-200 text-lg font-light max-w-2xl mx-auto drop-shadow" style={{ fontFamily: 'Playfair Display, serif' }}>Everything you need to know about Boldify Jewelry</p>
            <div className="w-24 h-px bg-[#C4A747] mx-auto mt-6"></div>
          </div>
          <div className="space-y-4">
            {faqItems.map((item, index) => (
              <div key={index} className="bg-black/40 backdrop-blur-md border border-white/20 rounded-lg overflow-hidden transition-all duration-300 hover:border-[#C4A747]/70 shadow-lg">
                <button onClick={() => toggleFaq(index)} className="w-full px-6 py-4 text-left flex justify-between items-center focus:outline-none group" aria-expanded={openFaqIndex === index}>
                  <span className="text-white font-semibold text-lg" style={{ fontFamily: 'Playfair Display, serif' }}>{item.question}</span>
                  <ChevronDown className={`h-5 w-5 text-[#C4A747] transition-transform duration-300 ${openFaqIndex === index ? 'rotate-180' : ''}`} />
                </button>
                <div className={`overflow-hidden transition-all duration-300 ${openFaqIndex === index ? 'max-h-96' : 'max-h-0'}`}>
                  <div className="px-6 pb-6 pt-2 text-gray-100 leading-relaxed border-t border-white/10">{item.answer}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Contact Section */}
      <section id="contact" ref={(el) => { sectionsRef.current[7] = el; contactRef.current = el; }} className="py-12 px-6 md:px-12 lg:px-24">
        <div className="max-w-6xl mx-auto">
          <div ref={(el) => (headingRefs.current[7] = el)} className="mb-12 text-center opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-4xl md:text-5xl font-semibold text-[#1A1A1A] mb-4" style={{ fontFamily: 'Jolt, serif' }}>Get In Touch</h2>
            <p className="text-[#4A4A4A] text-lg font-light max-w-2xl mx-auto" style={{ fontFamily: 'Playfair Display, serif' }}>We'd love to hear from you</p>
          </div>
          <div className="grid md:grid-cols-2 gap-16">
            <div className="space-y-10">
              <div className="flex items-start space-x-5"><Phone className="h-6 w-6 text-[#C4A747] mt-1 flex-shrink-0" /><div><h3 className="text-[#1A1A1A] font-bold mb-2 text-lg" style={{ fontFamily: 'Playfair Display, serif' }}>Phone</h3><a href="tel:+254798893450" className="text-[#666666] hover:text-[#C4A747] transition-colors duration-300" style={{ fontFamily: 'Marcellus, serif' }}>0798 893 450</a></div></div>
              <div className="flex items-start space-x-5"><Mail className="h-6 w-6 text-[#C4A747] mt-1 flex-shrink-0" /><div><h3 className="text-[#1A1A1A] font-bold mb-2 text-lg" style={{ fontFamily: 'Playfair Display, serif' }}>Email</h3><a href="mailto:boldifyjewelry@gmail.com" className="text-[#666666] hover:text-[#C4A747] transition-colors duration-300" style={{ fontFamily: 'Marcellus, serif' }}>boldifyjewelry@gmail.com</a></div></div>
              <div className="flex items-start space-x-5"><MessageCircle className="h-6 w-6 text-[#C4A747] mt-1 flex-shrink-0" /><div><h3 className="text-[#1A1A1A] font-bold mb-2 text-lg" style={{ fontFamily: 'Playfair Display, serif' }}>WhatsApp</h3><a href="https://wa.me/254798893450" target="_blank" rel="noopener noreferrer" className="inline-block bg-[#1A1A1A] text-white px-6 py-3 font-bold transition-all duration-300 hover:bg-[#C4A747] hover:text-black" style={{ fontFamily: 'Marcellus, serif' }}>CHAT WITH US</a></div></div>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div><input type="text" placeholder="Your Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-transparent border-b border-[#DDD] text-[#1A1A1A] px-0 py-3 focus:outline-none focus:border-[#C4A747] transition-colors duration-300 placeholder:text-[#AAA]" style={{ fontFamily: 'Marcellus, serif' }} required /></div>
              <div><input type="email" placeholder="Your Email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full bg-transparent border-b border-[#DDD] text-[#1A1A1A] px-0 py-3 focus:outline-none focus:border-[#C4A747] transition-colors duration-300 placeholder:text-[#AAA]" style={{ fontFamily: 'Marcellus, serif' }} required /></div>
              <div><textarea placeholder="Your Message" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={5} className="w-full bg-transparent border-b border-[#DDD] text-[#1A1A1A] px-0 py-3 focus:outline-none focus:border-[#C4A747] transition-colors duration-300 resize-none placeholder:text-[#AAA]" style={{ fontFamily: 'Marcellus, serif' }} required /></div>
              <button type="submit" className="group relative w-full bg-[#1A1A1A] text-white px-6 py-3 font-bold overflow-hidden transition-all duration-300 hover:bg-[#C4A747] hover:text-black" style={{ fontFamily: 'Marcellus, serif' }}>
                <span className="relative z-10 flex items-center justify-center space-x-2"><span>SEND MESSAGE</span><Send className="h-5 w-5" /></span>
  ...
  <p className="text-gray-500 text-xs" style={{ fontFamily: 'Marcellus, serif' }}>
    ...
    >
      ...
      <style>{`
        ...
        .bg-radial-gradient { background: radial-gradient(circle at 50% 50%, rgba(196, 167, 71, 0.3) 0%, transparent 70%); }
        @keyframes smokeMove { 0% { background-position: 0% 0%; } 100% { background-position: 200% 200%; } }
        .animate-smokeMove { animation: smokeMove 20s linear infinite; }
        .animate-smoke { animation: smokeFade 8s ease-in-out infinite alternate; }
        @keyframes smokeFade { 0% { opacity: 0.2; } 100% { opacity: 0.5; } }
      `}</style>
    </div>
  );
}
