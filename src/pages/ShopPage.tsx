            ...
            {filteredProducts.map(product => (
              <div key={product.id} className="group cursor-pointer overflow-hidden bg-gray-900/50 border border-gray-800 hover:border-gold transition-all duration-300">
                <div className="relative h-64 md:h-80 overflow-hidden">
                  ...
                  <button onClick={(e) => toggleWishlist(product.id, e)} className="absolute top-4 right-4 bg-black/50 hover:bg-black p-2 rounded-full transition-colors z-10" aria-label={wishlist.has(product.id) ? 'Remove from wishlist' : 'Add to wishlist'}>
