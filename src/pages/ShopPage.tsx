...
export default function ShopPage({ onWishlistChange, initialCategory }: ShopPageProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory) {
      ...
      if (category && !category.id.includes('temp')) {
        ...
        // Temporary categories (Watches, Rings) have no products yet
        filtered = [];
      }
    }
  ...
  const availableSubcategories = selectedCategory
    ? subcategories.filter((s) => {
        const category = categories.find((c) => c.slug === selectedCategory);
        return category && s.category_id === category.id && !category.id.includes('temp');
      })
    : [];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black pt-20">
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className="group cursor-pointer overflow-hidden bg-gray-900/50 border border-gray-800 hover:border-gold transition-all duration-300">
                <div className="relative h-64 md:h-80 overflow-hidden">
                  ...
                  <button onClick={() => toggleWishlist(product.id)} className="absolute top-4 right-4 bg-black/50 hover:bg-black p-2 transition-colors duration-300" aria-label={wishlist.has(product.id) ? 'Remove from wishlist' : 'Add to wishlist'}>
      ...
      <footer className="bg-black border-t border-gold/20 py-8 px-4 mt-20">
        <div className="max-w-7xl mx-auto text-center"><p className="text-gray-400" style={{ fontFamily: 'Marcellus, serif' }}>© 2026 Boldify Jewellery.Ke. All rights reserved.</p></div>
      </footer>
    </div>
  );
}
