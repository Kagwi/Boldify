...
import { Search, X, Heart, Filter, Instagram, Facebook } from 'lucide-react';
...
export default function ShopPage({ onWishlistChange, initialCategory }: ShopPageProps) {
  ...
  // --- Wishlist: load from localStorage on mount ---
  useEffect(() => {
  ...
  // --- Data fetching (unchanged, but with error handling) ---
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory) {
      ...
      if (category && !category.id.includes('temp')) {
        filtered = filtered.filter(p => p.category_id === category.id);
      } else if (category && category.id.includes('temp')) {
        filtered = []; // temporary categories have no products
      }
    }
        ...
        {showFilters && (
          <div className="mb-8 bg-gray-900/50 border border-gray-800 p-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div>
                <label className="block text-white mb-3 font-bold" style={{ fontFamily: 'Marcellus, serif' }}>
                  Category
                </label>
                <select
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!selectedCategory || selectedCategory === 'watches' || selectedCategory === 'rings'}
              ...
              >
                <div className="relative h-64 md:h-80 overflow-hidden">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 bg-black/50 hover:bg-black p-2 rounded-full transition-colors z-10"
