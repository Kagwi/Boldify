...
interface NavbarProps {
  ...
  onSearchClick?: () => void;      // kept for compatibility, but we'll use internal modal
  wishlistCount: number;
}

export default function Navbar({ currentPage, onNavigate, wishlistCount: externalWishlistCount }: NavbarProps) {
  ...
  // Read wishlist from localStorage and map to product objects
  const loadWishlist = () => {
  ...
  // Load wishlist when allProducts are ready, and when localStorage changes
  useEffect(() => {
    loadWishlist();
  }, [allProducts]);
  ...
  const removeFromWishlist = (productId: string) => {
    ...
    // Trigger storage event for other components (HomePage/ShopPage)
    window.dispatchEvent(new StorageEvent('storage', { key: 'boldify_wishlist', newValue: JSON.stringify(newIds) }));
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-start justify-center pt-20 px-4">
      <div className="bg-gray-900 w-full max-w-2xl rounded-lg shadow-2xl border border-gold/30">
        <div className="flex justify-between items-center p-4 border-b border-gray-800">
          <h2 className="text-xl font-bold text-gold" style={{ fontFamily: 'Jolt, serif' }}>Search Products</h2>
          <button onClick={() => setIsSearchModalOpen(false)} className="text-gray-400 hover:text-gold">
            <X className="h-6 w-6" />
          </button>
        </div>
        <div className="p-4">
          <input
            ...
            className="w-full bg-gray-800 border border-gray-700 text-white px-4 py-3 rounded focus:outline-none focus:border-gold"
            autoFocus
          />
            ...
            {searchResults.map(product => (
              <div key={product.id} className="flex items-center gap-4 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                ...
                <a
                  ...
                  className="bg-gold text-black px-3 py-2 text-sm font-bold rounded hover:bg-gold/90 transition"
                >
                  ORDER
                </a>
              </div>
  ...
  const WishlistModal = () => (
    <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-start justify-center pt-20 px-4">
      <div className="bg-gray-900 w-full max-w-3xl rounded-lg shadow-2xl border border-gold/30">
        <div className="flex justify-between items-center p-4 border-b border-gray-800">
          <h2 className="text-xl font-bold text-gold" style={{ fontFamily: 'Jolt, serif' }}>
            Your Wishlist ({wishlistProducts.length})
          </h2>
          <button onClick={() => setIsWishlistModalOpen(false)} className="text-gray-400 hover:text-gold">
            <X className="h-6 w-6" />
          </button>
        </div>
                ...
                <div key={product.id} className="flex items-center gap-4 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="bg-red-600/20 text-red-400 px-3 py-2 text-sm font-bold rounded hover:bg-red-600/40 transition flex items-center gap-1"
                    >
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center space-x-6">
              ...
              <button
                ...
                className="relative text-gray-300 hover:text-gold transition-colors duration-300"
              >
              ...
              <a
                href="https://www.instagram.com/boldify_jewellery.ke?utm_source=qr&igsh=cTZ4c2ljcmRoNTJs"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-300 hover:text-gold transition-colors duration-300"
              >
              ...
              <a
                href="https://www.tiktok.com/@boldify_jewellery?_r=1&_t=ZS-94PQWhub0XR"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-300 hover:text-gold transition-colors duration-300"
              >
          ...
          <div className="md:hidden bg-black/98 border-t border-gold/20">
            <div className="px-4 py-6 space-y-4">
              ...
              <div className="flex space-x-6 pt-4 border-t border-gold/20">
                <button onClick={() => { setIsSearchModalOpen(true); setIsMobileMenuOpen(false); }}>
                  <Search className="h-6 w-6 text-gray-300 hover:text-gold" />
                </button>
                <button onClick={() => { setIsWishlistModalOpen(true); setIsMobileMenuOpen(false); }} className="relative">
                  <Heart className="h-6 w-6 text-gray-300 hover:text-gold" />
                  {wishlistIds.size > 0 && (
                    <span className="absolute -top-2 -right-2 bg-gold text-black text-xs rounded-full h-5 w-5 flex items-center justify-center font-bold">
                      {wishlistIds.size}
                    </span>
                ...
                <a href="https://www.instagram.com/boldify_jewellery.ke?utm_source=qr&igsh=cTZ4c2ljcmRoNTJs" target="_blank" rel="noopener noreferrer">
                  <Instagram className="h-6 w-6 text-gray-300 hover:text-gold" />
                </a>
                <a href="https://www.tiktok.com/@boldify_jewellery?_r=1&_t=ZS-94PQWhub0XR" target="_blank" rel="noopener noreferrer">
                  <svg className="h-6 w-6 fill-gray-300 hover:fill-gold" viewBox="0 0 24 24">
                    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
                  </svg>
                </a>
              </div>
